...
public class Table {
     ...
     *            The size of the big blind.
     */
    public Table(TableType type, TableObserver tableObserver, int bigBlind) {
        this.tableType = type;
     ...
     */
    public void run() {
        tableObserver.joinedTable(tableType, bigBlind, players);
        dealerPosition = -1;
        actorPosition = -1;
        while (true) {
    ...
    private void resetHand() {
        ...
        tableObserver.handStarted(dealer);
        for (Player player : players) {
            player.getClient().handStarted(player.getCash());
        }
        notifyPlayersUpdated(false);
    ...
    private void rotateActor() {
        ...
        actor = activePlayers.get(actorPosition);
        tableObserver.actorRotated(actor);
    }
...
//        System.out.format("[DEBUG]  Total: %d\n", getTotalPot());
        ...
        for (Player playerToShow : showingPlayers) {
            ...
            if (doShow) {
                // Show hand.
                tableObserver.playerUpdated(playerToShow);
                ...
                tableObserver.playerUpdated(playerToShow.publicClone());
...
//                                System.out.format("[DEBUG] %s receives an odd chip from the pot.\n", winner);
    ...
    private void notifyMessage(String message, Object... args) {
        ...
        tableObserver.messageReceived(message);
    }
    ...
    private void notifyBoardUpdated() {
        int pot = getTotalPot();
        tableObserver.boardUpdated(board, bet, pot);
    }
    ...
    private void notifyPlayersUpdated(boolean showdown) {
        for (Player player : players) {
            ...
            tableObserver.playerUpdated(player);
        }
    }
    ...
    private void notifyPlayerActed() {
        tableObserver.playerActed(actor.publicClone());
    }
